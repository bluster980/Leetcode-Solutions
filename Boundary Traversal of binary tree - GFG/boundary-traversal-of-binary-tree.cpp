...
public:
    ...
    void right(Node *root,vector<int> &v){
        Node *dumy = root->right;
        vector<int> vv;
        while(dumy){
            if(!isLeaf(dumy)) vv.push_back(dumy->data);
            if(dumy->right) dumy = dumy->right;
            else dumy=dumy->left;
        }
    ...
    void cur(Node *root,vector<int> &v){
        if(isLeaf(root)) {v.push_back(root->data); return;}
        if(root->left) cur(root->left,v);
        if(root->right) cur(root->right,v);
    }
